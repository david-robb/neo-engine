...
export const initializePlanets = (simulationDate: Date): EnginePrimaryBody[] => {
    return [buildSun(), buildEarth(simulationDate), buildMars(simulationDate), buildMercury(simulationDate)];
};
...
export const initializePlanetMeshes = (
    ...
    const orbitMeshes = planets
        ...
        .map((planet) => buildOrbitMeshLine(planet.orbitData!, planet.color, planet.name, 500));
...
const buildEarth = (simulationDate: Date): EnginePrimaryBody => {
    const orbitalData = {
    ...
    const epochDate = parse('2000-01-01 12:00:00');
    ...
    return {
        ...
        epochOffset,
        color: new Color(0x1c96c5),
        radiusKm: 6357 * 100,
        currentPosition: new Vector3(0, 0, 0),
    };
};

const buildMars = (simulationDate: Date): EnginePrimaryBody => {
    const orbitalData = {
    ...
    return {
        ...
        orbitData: orbitalData,
        epochOffset,
        color: new Color(0xac7d6f),
        radiusKm: 2439.7 * 100,
        currentPosition: new Vector3(0, 0, 0),
    };
};

const buildMercury = (simulationDate: Date): EnginePrimaryBody => {
    const orbitalData = {
        orbitalPeriod: 7603200,
        ascendingNodeLongitude: degreesToRadians(48.34),
        eccentricity: 0.20564,
        inclination: degreesToRadians(7.006),
        semiMajorAxis: 57909335.7,
        meanAnomaly: degreesToRadians(174.79),
        perihelionArgument: degreesToRadians(77.46) - degreesToRadians(48.34),
