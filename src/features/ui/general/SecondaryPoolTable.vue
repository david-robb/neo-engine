...
<template>
    <div class="h-full w-full">
        <Drawer v-model:visible="showDetails" :header="selected?.name ?? ''" position="right" class="opacity-90 md:w-80! lg:w-120!">
            <div>
        ...
        <div class="absolute top-0 right-0 z-10">
            <DataTable v-if="state.focusedPoolArray.length" :value="state.focusedPoolArray" class="opacity-80 m-6 p-datatable-sm">
                <Column field="name" header="Name" class="text-xs"></Column>
                <Column field="distanceToEarth" header="Earth Distance (KM)" class="text-xs">
                    <template #body="slotProps">
                        {{ slotProps.data.distanceToEarth.toFixed(2) }}
                    </template>
                </Column>
                <Column field="velocity" header="Velocity (KM/S)" class="text-xs">
                    <template #body="slotProps">
                ...
                <Column>
                    <template #body="{ data }">
                        <Button size="small" icon="pi pi-trash" @click="deleteFromPool(data)" severity="secondary" rounded></Button>
                    </template>
                </Column>
                <Column>
                    <template #body="{ data }">
